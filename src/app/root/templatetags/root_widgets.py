...
def home_page_discussion_widget(context):
    ...
    if user.is_authenticated() and any(
       [user.is_admin, user.is_ni_nyampinga_journalist, user.is_ambassador]):
        try:
            ni_nyampinga_discussion = discussion_models.Discussion.objects\
                .permitted()\
    ...
    discussions = list(discussion_models.Discussion.objects.permitted()\
        .filter(for_ni_nyampinga_journalists_only=False, featured=True)[:4])
