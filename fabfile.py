...
def _email_project_deployed(instance_type):
    ...
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "[Deploy] [%s] instance %s by %s" % (
...
def _get_local_settings(instance_type):
    ...
    settings_dict = {
        ...
        'name': (instance_type in ['dev', 'qa']
                 and '%s_%s' % (PROJECT_NAME, instance_type)
                 or PROJECT_NAME),
        'user': PROJECT_NAME,
    ...
    settings_list.append('DEBUG = %s' % ('False' if instance_type == 'master'
                                         else 'True'))
    ...
    settings_list.append("BROKER_URL = 'amqp://%s:%s@127.0.0.1:5672//%s'" % (
...
def pg_user_exists(name):
    ...
    with settings(hide('running', 'stdout', 'stderr', 'warnings'),
                  warn_only=True):
        res = _run_as_pg('''psql -t -A -c "SELECT COUNT(*) FROM pg_user '''
                         '''WHERE usename = '%(name)s';"''' % locals())
    return (res == "1")
    ...
    with settings(warn_only=True):
        env.run_cmd(
            'sudo rabbitmqctl add_user %s %s' % (
                instanced_project_name, instanced_project_name) + ' && '
...
def symlink_nginx(server_name):
    with settings(warn_only=True):
        ...
        env.run_cmd('sudo ln -s $PWD/nginx/%s.conf '
                    '/etc/nginx/sites-available/%s.conf' % (
                        server_name, server_name))
        env.run_cmd('sudo rm /etc/nginx/sites-enabled/%s' % server_name)
        env.run_cmd('sudo ln -s $PWD/nginx/%s.conf '
                    '/etc/nginx/sites-enabled/%s.conf' % (
                        server_name, server_name))


def symlink_supervisor_gunicorn(server_name):
    with settings(warn_only=True):
        env.run_cmd('sudo rm /etc/supervisor/conf.d/%s.gunicorn.conf' % (
            server_name,))
        env.run_cmd('sudo ln -s $PWD/supervisor/gunicorn.conf '
                    '/etc/supervisor/conf.d/%s.gunicorn.conf' % server_name)


def symlink_supervisor_celeryd(server_name):
    with settings(warn_only=True):
        env.run_cmd('sudo rm /etc/supervisor/conf.d/%s.celeryd.conf' % (
            server_name,))
        env.run_cmd('sudo ln -s $PWD/supervisor/celeryd.conf '
                    '/etc/supervisor/conf.d/%s.celeryd.conf' % server_name)
...
def setup_postgres_database(instanced_project_name):
    with settings(warn_only=True):
        ...
        if not pg_database_exists(instanced_project_name):
            _run_as_pg('createdb -O %s %s' % (
                PROJECT_NAME, instanced_project_name))
...
def create_settings_local_file(instance_type):
    with settings(warn_only=True):
        if env.run_cmd("test -d src/project/settings_local.py").failed:
            ...
            env.run_cmd(
                'echo "%s" > src/project/settings_local.py' % (
                    _get_local_settings(instance_type),)
            )
...
def test_repo_exists(code_dir):
    with settings(warn_only=True):
        if env.run_cmd("test -d %s" % code_dir).failed:
            env.run_cmd("git clone git@git.unomena.net:%s.git %s" % (
                REPO_PATH, code_dir))
